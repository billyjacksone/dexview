...
const SidebatListItem : React.FC<SidebarListItemProps> = ({name, Icon, src, color, link}) => {


  return(
    <Link href={link} className='flex w-full justify-start gap-2 items-center'>
...
const SidebarListItems = [{name: 'Advertise', Icon: Advertise, color: "#d0d1d2", link: '/advertise'},
    ...
    <div className='flex flex-col bg-[#1e2025] text-white w-[15%] mt-3 py-1 items-center gap-3'>
        ...
        <div className='flex w-full px-3 flex-col gap-4'>
          {SidebarListItems.map((item, index) => (
            <SidebatListItem key={index} name={item.name} Icon={item.Icon} color={item.color} link={item.link} src={item.src ? item.src : ''} />
          ))}
        </div>
    </div>
  )
