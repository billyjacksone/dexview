...
const SidebarListItem: React.FC<SidebarListItemProps> = ({ name, Icon, src, color, link, showIconsOnly }) => {
  return (
    <Link href={link}>
      <div
        className={`flex items-center gap-1 text-gray-300 p-2 rounded-md transition-all hover:bg-[#b7f357] `}
        style={{ background: 'rgb(22, 26, 30)' }}
        onMouseEnter={(e) => (e.currentTarget.style.background = 'rgba(169, 169, 169, 0.1)')}
        onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}
      >
        {Icon && <Icon className={`w-8 h-8 ${showIconsOnly ? 'mr-10' : ''}`} />}
        {src && <Image alt="Icon" src={src} width={26} height={26} />}
        <p className={`opacity-80 text-xs ${showIconsOnly ? 'hidden' : ''} ${isBlueText(name) ? 'text-blue-500' : ''} `}>
          {name}
        </p>
      </div>
    </Link>
...
const Sidebar: React.FC = () => {
  ...
  return (
    <div className={`flex flex-col bg-[#1e2025] text-white w-fit mt-3 items-center gap-1`} style={{ background: 'rgb(22, 26, 30)' }}>
      <div
        className={`flex w-[95%] border-b-[1px] border-[rgb(39,42,42)] py-1 items-center cursor-pointer`}
